...
mod tests {
    ...
    fn pre_execution_rejects_non_rewards_beneficiary() {
        let body = OutbeBlockBody {
            transactions: vec![signed_system_tx(
                &outbe_evm::OutbeEvmSigner::from_secret_bytes(outbe_primitives::test_keys::secret(
                    (4u8) as u64,
                ))
                .unwrap(),
    ...
    fn pre_execution_rejects_finalization_metadata_for_non_parent_hash() {
        let signer =
            outbe_evm::OutbeEvmSigner::from_secret_bytes(outbe_primitives::test_keys::secret(3))
                .unwrap();
