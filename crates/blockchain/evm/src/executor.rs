...
mod tests {
    ...
    fn test_evm_signer() -> Arc<OutbeEvmSigner> {
        Arc::new(OutbeEvmSigner::from_secret_bytes(outbe_primitives::test_keys::secret(1)).unwrap())
    }
    ...
    fn whole_committee_ocomp_deadline_and_successor_blocks_execute() {
        ...
        let first_signer = Arc::new(
            OutbeEvmSigner::from_secret_bytes(outbe_primitives::test_keys::secret(0x21)).unwrap(),
        );
        let second_signer = Arc::new(
    ...
    fn boundary_activation_allows_registered_next_epoch_proposer() {
        let signer = test_evm_signer();
        let proposer = signer.address();
        let old_active_secret = outbe_primitives::test_keys::secret(2);
    ...
    fn independent_body_stores_produce_identical_full_block_state_receipts_and_balances() {
        ...
        let seed_state = || {
            ...
            let state = state_with_active_validators_seeded_at_block(
                ...
                1,
                |storage| {
                    // Qualification must see a fresh quote so this block really
                    // mutates the bucket and exercises pending-state cleanup.
                    outbe_oracle::api::set_exchange_rate(
                        storage.clone(),
                        Address::ZERO,
                        outbe_oracle::api::DAY_TYPE_PAIR,
    ...
    fn verifier_rejects_begin_system_tx_signer_mismatch() {
        ...
        let wrong_signer = Arc::new(
            OutbeEvmSigner::from_secret_bytes(outbe_primitives::test_keys::secret(2)).unwrap(),
        );
        let mut state = state_with_active_proposer(proposer);
        let evm_env = test_evm_env(1, REWARDS_ADDRESS);
        let config =
    ...
    fn bootstrap_test_tx() -> reth_primitives_traits::Recovered<reth_ethereum::TransactionSigned> {
        let key = k256::ecdsa::SigningKey::from_slice(&outbe_primitives::test_keys::secret(0x11))
            .unwrap();
