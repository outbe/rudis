...
mod tests {
    ...
    fn system_tx_leader_binding_accepts_consensus_leader_address() {
        let (keys, _) = participants();
        let signer =
            OutbeEvmSigner::from_secret_bytes(outbe_primitives::test_keys::secret(7)).unwrap();
    ...
    fn system_tx_leader_binding_uses_epoch_registered_committee() {
        let (keys, _) = participants();
        let signer =
            OutbeEvmSigner::from_secret_bytes(outbe_primitives::test_keys::secret(9)).unwrap();
        let mut validator_set = validator_set_from_keys(&keys);
    ...
    fn system_tx_leader_binding_rejects_non_leader_signer() {
        ...
        let leader_signer =
            OutbeEvmSigner::from_secret_bytes(outbe_primitives::test_keys::secret(7)).unwrap();
        let non_leader_signer =
            OutbeEvmSigner::from_secret_bytes(outbe_primitives::test_keys::secret(8)).unwrap();
        let mut validator_set = validator_set_from_keys(&keys);
