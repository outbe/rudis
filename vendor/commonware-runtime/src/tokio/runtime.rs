...
impl crate::Runner for Runner {
        ...
        Fut: Future,
    {
        ...
        let output = runtime.block_on(panicked.interrupt(f(context)));
        gauge.dec();
...
mod tests {
    ...
    #[test]
    fn test_runner_waits_for_shared_task_cleanup() {
        for blocking in [false, true] {
            ...
            let runner = std::thread::spawn(move || {
                Runner::new(cfg).start(move |context| async move {
                    let mut parent = context.child("parent").spawn(move |parent| async move {
                        parent.shared(blocking).spawn(move |_| async move {
                            let _drop_gate = TaskDropGate {
                                entered: drop_entered_tx,
                                release: drop_release_rx,
                            };
            ...
            drop_entered_rx
                .recv_timeout(Duration::from_secs(5))
                .unwrap_or_else(|_| panic!("Runner did not cancel shared({blocking}) task"));
            assert!(
                runner_returned_rx.try_recv().is_err(),
                "Runner returned before shared task cleanup"
            );
            drop_release_tx.send(()).unwrap();
            runner_returned_rx
                .recv_timeout(Duration::from_secs(5))
                .expect("Runner did not return after shared task cleanup");
            runner.join().unwrap();
            let _ = std::fs::remove_dir_all(storage_directory);
        }
    }
